...
# ============================================================
# SÉQUENCES SPATIO-TEMPORELLES (CNN-GRU)
# Shape : (N, T, C, pH, pW)
# ============================================================
def prepare_sequences_tile_cnn_gru(temporal_data, r0, r1, c0, c1, patch_size=PATCH_SIZE):
    ...
    for target_idx in range(SEQUENCE_LENGTH, num_months):
        ...
        for pr in range(0, H_tile - patch_size + 1, patch_size):
            for pc in range(0, W_tile - patch_size + 1, patch_size):
                sub_seq = seq_stack[:, :, pr:pr+patch_size, pc:pc+patch_size]  # (T, C, pH, pW)
...
# ============================================================
# NORMALISATION  (N, T, C, pH, pW)
# ============================================================
def normalize_features_cnn_gru(X_train, X_val, X_test):
...
# DATASET
# ============================================================
class CDIDatasetCNNGRU(Dataset):
...
# ============================================================
# MODÈLE HYBRIDE CNN-GRU
# ============================================================
# Pour chaque pas de temps t :
#   CNN spatial  →  vecteur de features h_t
# Séquence (h_0 … h_{T-1})  →  GRU  →  dernière sortie cachée
# Projection  →  num_classes * pH * pW  →  reshape carte
# ============================================================
class SpatialCNNEncoder(nn.Module):
    """CNN spatial partagé dans le temps : (B, C, pH, pW) → (B, cnn_out_dim)"""
...
class CDICNNGRUClassifier(nn.Module):
    ...
    def forward(self, x):
        ...
        # CNN appliqué à chaque pas de temps
        ...
        # GRU temporel
        gru_out, _ = self.gru(cnn_feats)             # (B, T, hidden)
        h_last     = gru_out[:, -1, :]               # (B, hidden)
...
def process_tile(temporal_data, tile_idx, r0, r1, c0, c1):
    seqs, tgts, dates, origins = prepare_sequences_tile_cnn_gru(
    ...
    X_tr_s, X_va_s, X_te_s = normalize_features_cnn_gru(X_tr, X_va, X_te)
    del X_tr, X_va, X_te; gc.collect()

    tr_loader = DataLoader(CDIDatasetCNNGRU(X_tr_s, y_tr), BATCH_SIZE, shuffle=False, pin_memory=True)
    va_loader = DataLoader(CDIDatasetCNNGRU(X_va_s, y_va), BATCH_SIZE, shuffle=False, pin_memory=True)
    te_loader = DataLoader(CDIDatasetCNNGRU(X_te_s, y_te), BATCH_SIZE, shuffle=False, pin_memory=True)
    del X_tr_s, X_va_s, X_te_s; gc.collect()

    model = CDICNNGRUClassifier(
...
def print_global_summary(results, full_shape, output_dir='.'):
    ...
    print("\n" + "="*70)
    print(f"RESUME GLOBAL (CNN-GRU) — {len(df)} tuile(s)  |  {total_samples:,} pixels test")
    print("="*70)
    ...
    df.to_csv(os.path.join(output_dir, 'tile_results_cnn_gru.csv'), index=False)
    print("\nRésultats détaillés → tile_results_cnn_gru.csv")
...
if __name__ == '__main__':
    OUTPUT_DIR = '/kaggle/working/cdi_rasters_cnn_gru'
    os.makedirs(OUTPUT_DIR, exist_ok=True)

    print("\n" + "="*70)
    print("🚀 CDI PREDICTION — CNN-GRU Hybride (raster Tunisie complet)")
    ...
    print(f"   Couches GRU               : {NUM_LAYERS}")
    print("="*70)
