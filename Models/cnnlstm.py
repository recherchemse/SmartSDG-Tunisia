...
# CNN channels
CNN_CHANNELS    = [32, 64]   # canaux des couches Conv2D successives
...
# ============================================================
# SÉQUENCES SPATIO-TEMPORELLES POUR UNE TUILE (CNN-LSTM)
# Chaque pas de temps : patch (C, pH, pW) conservé en 2D pour le CNN
# Shape finale : (N, T, C, pH, pW)
# ============================================================
def prepare_sequences_tile_cnn_lstm(temporal_data, r0, r1, c0, c1, patch_size=PATCH_SIZE):
    ...
    for target_idx in range(SEQUENCE_LENGTH, num_months):
        ...
        for t, md in enumerate(sequence_months):
            for c_idx, k in enumerate(INDICATOR_KEYS):
                ...
                if (np.any(np.isnan(patch_data)) or
                        np.any(~np.isfinite(patch_data)) or
                        np.any(np.abs(patch_data) > 1e6)):
                    valid_global = False
                    break
        ...
        for pr in range(0, H_tile - patch_size + 1, patch_size):
            for pc in range(0, W_tile - patch_size + 1, patch_size):
                # (T, C, pH, pW) — on garde la structure 2D pour le CNN
...
# ============================================================
# NORMALISATION (par canal spectral, indépendamment de H/W)
# X shape : (N, T, C, pH, pW)
# ============================================================
def normalize_features_cnn_lstm(X_train, X_val, X_test):
    N, T, C, pH, pW = X_train.shape
    # Reshape en (N*T*pH*pW, C) pour RobustScaler
    def _reshape_for_scaler(X):
        n = X.shape[0]
        return X.transpose(0, 1, 3, 4, 2).reshape(-1, C)   # (..., C)

    scaler  = RobustScaler()
    X_tr_rs = scaler.fit_transform(_reshape_for_scaler(X_train))
    X_va_rs = scaler.transform(_reshape_for_scaler(X_val))
    X_te_rs = scaler.transform(_reshape_for_scaler(X_test))

    def _restore(X_rs, n):
        return X_rs.reshape(n, T, pH, pW, C).transpose(0, 1, 4, 2, 3)  # (N,T,C,pH,pW)

    return (_restore(X_tr_rs, X_train.shape[0]).astype(np.float32),
            _restore(X_va_rs, X_val.shape[0]).astype(np.float32),
            _restore(X_te_rs, X_test.shape[0]).astype(np.float32))


# ============================================================
# DATASET
# ============================================================
class CDIDatasetCNNLSTM(Dataset):
    def __init__(self, X, y):
        self.X = torch.FloatTensor(X)   # (N, T, C, pH, pW)
...
# ============================================================
# Architecture :
#   Pour chaque pas de temps t ∈ [0, T-1] :
#       CNN extrait un vecteur de features spatial  → h_t  (feature vector)
#   La séquence (h_0, ..., h_{T-1}) alimente le LSTM
#   La dernière sortie cachée est projetée sur num_classes * pH * pW
# ============================================================
class SpatialCNNEncoder(nn.Module):
    """Encodeur CNN partagé dans le temps : (B, C, pH, pW) → (B, cnn_out_dim)"""
    def __init__(self, in_channels, cnn_channels, dropout):
        super().__init__()
        layers = []
        ch_in  = in_channels
        for ch_out in cnn_channels:
            ...
            ch_in = ch_out
        layers.append(nn.AdaptiveAvgPool2d(1))   # → (B, ch_out, 1, 1)
        self.cnn     = nn.Sequential(*layers)
        self.dropout = nn.Dropout(dropout)
        self.out_dim = cnn_channels[-1]

    def forward(self, x):                        # x : (B, C, pH, pW)
        feat = self.cnn(x)                       # (B, ch_out, 1, 1)
        feat = feat.view(feat.size(0), -1)       # (B, ch_out)
        return self.dropout(feat)


class CDICNNLSTMClassifier(nn.Module):
    def __init__(self, in_channels=NUM_FEATURES,
        ...
        # Encodeur CNN spatial partagé
        self.cnn_encoder = SpatialCNNEncoder(in_channels, cnn_channels, dropout)
        cnn_out_dim      = self.cnn_encoder.out_dim
        ...
        # Tête de classification → num_classes * pH * pW
    ...
    def forward(self, x):
        ...
        # Appliquer le CNN à chaque pas de temps → (B, T, cnn_out_dim)
        x_reshaped = x.view(B * T, C, pH, pW)          # (B*T, C, pH, pW)
        cnn_feats  = self.cnn_encoder(x_reshaped)       # (B*T, cnn_out_dim)
        cnn_feats  = cnn_feats.view(B, T, -1)           # (B, T, cnn_out_dim)
        ...
        # Classification
        logits = self.classifier(h_last)                # (B, num_classes*pH*pW)
        logits = logits.view(B, self.num_classes, self.patch_size, self.patch_size)
        return logits                                   # (B, num_classes, pH, pW)
...
def process_tile(temporal_data, tile_idx, r0, r1, c0, c1):
    seqs, tgts, dates, origins = prepare_sequences_tile_cnn_lstm(
    ...
    X_tr_s, X_va_s, X_te_s = normalize_features_cnn_lstm(X_tr, X_va, X_te)
    del X_tr, X_va, X_te; gc.collect()

    tr_loader = DataLoader(CDIDatasetCNNLSTM(X_tr_s, y_tr), BATCH_SIZE, shuffle=False, pin_memory=True)
    va_loader = DataLoader(CDIDatasetCNNLSTM(X_va_s, y_va), BATCH_SIZE, shuffle=False, pin_memory=True)
    te_loader = DataLoader(CDIDatasetCNNLSTM(X_te_s, y_te), BATCH_SIZE, shuffle=False, pin_memory=True)
    del X_tr_s, X_va_s, X_te_s; gc.collect()

    model = CDICNNLSTMClassifier(
...
def print_global_summary(results, full_shape, output_dir='.'):
    ...
    print("\n" + "="*70)
    print(f"RESUME GLOBAL (CNN-LSTM) — {len(df)} tuile(s)  |  {total_samples:,} pixels test")
    print("="*70)
    ...
    df.to_csv(os.path.join(output_dir, 'tile_results_cnn_lstm.csv'), index=False)
    print("\nRésultats détaillés → tile_results_cnn_lstm.csv")
...
if __name__ == '__main__':
    OUTPUT_DIR = '/kaggle/working/cdi_rasters_cnn_lstm'
    os.makedirs(OUTPUT_DIR, exist_ok=True)

    print("\n" + "="*70)
    print("🚀 CDI PREDICTION — CNN-LSTM Hybride (raster Tunisie complet)")
    ...
    print("\n🔄 Traitement des tuiles (CNN-LSTM)...\n")
    all_results = []
