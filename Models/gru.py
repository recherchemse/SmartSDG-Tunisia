...
# ============================================================
# SÉQUENCES SPATIO-TEMPORELLES POUR UNE TUILE (GRU)
# Chaque patch est aplati en vecteur : T x (C * pH * pW)
# ============================================================
def prepare_sequences_tile_gru(temporal_data, r0, r1, c0, c1, patch_size=PATCH_SIZE):
    ...
    for target_idx in range(SEQUENCE_LENGTH, num_months):
        ...
        for pr in range(0, H_tile - patch_size + 1, patch_size):
            for pc in range(0, W_tile - patch_size + 1, patch_size):
                # seq : (T, C, pH, pW) → aplatir en (T, C*pH*pW) pour le GRU
...
# ============================================================
def normalize_features_gru(X_train, X_val, X_test):
...
# ============================================================
# MODÈLE GRU + TÊTE DE CLASSIFICATION PIXEL
# ============================================================
class CDIGRUClassifier(nn.Module):
    def __init__(self, input_size, hidden_size=HIDDEN_SIZE,
        ...
        # Tête : prédit num_classes * pH * pW valeurs pour reconstruire la carte
...
def process_tile(temporal_data, tile_idx, r0, r1, c0, c1):
    seqs, tgts, dates, origins = prepare_sequences_tile_gru(
    ...
    X_tr_s, X_va_s, X_te_s = normalize_features_gru(X_tr, X_va, X_te)
    del X_tr, X_va, X_te; gc.collect()

    input_size = X_tr_s.shape[2]  # C * pH * pW

    tr_loader = DataLoader(CDIDatasetGRU(X_tr_s, y_tr), BATCH_SIZE, shuffle=False, pin_memory=True)
    va_loader = DataLoader(CDIDatasetGRU(X_va_s, y_va), BATCH_SIZE, shuffle=False, pin_memory=True)
    te_loader = DataLoader(CDIDatasetGRU(X_te_s, y_te), BATCH_SIZE, shuffle=False, pin_memory=True)
    del X_tr_s, X_va_s, X_te_s; gc.collect()

    model = CDIGRUClassifier(
        input_size   = input_size,
        hidden_size  = HIDDEN_SIZE,
        num_layers   = NUM_LAYERS,
        num_classes  = 6,
...
def print_global_summary(results, full_shape, output_dir='.'):
    ...
    print("\n" + "="*70)
    print(f"RESUME GLOBAL (GRU) — {len(df)} tuile(s)  |  {total_samples:,} pixels test")
    print("="*70)
    ...
    df.to_csv(os.path.join(output_dir, 'tile_results_gru.csv'), index=False)
    print("\nRésultats détaillés → tile_results_gru.csv")
...
if __name__ == '__main__':
    OUTPUT_DIR = '/kaggle/working/cdi_rasters_gru'
    os.makedirs(OUTPUT_DIR, exist_ok=True)

    print("\n" + "="*70)
    print("🚀 CDI PREDICTION — GRU (raster Tunisie complet)")
