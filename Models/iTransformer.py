...
# Hyper-paramètres
SEQUENCE_LENGTH = 3
D_MODEL         = 64    # dimension d'embedding iTransformer
...
# ============================================================
# SÉQUENCES SPATIO-TEMPORELLES POUR UNE TUILE (iTransformer)
# Chaque patch est aplati en vecteur : T x (C * pH * pW)
# ============================================================
def prepare_sequences_tile(temporal_data, r0, r1, c0, c1, patch_size=PATCH_SIZE):
    ...
    for target_idx in range(SEQUENCE_LENGTH, num_months):
        ...
        for pr in range(0, H_tile - patch_size + 1, patch_size):
            for pc in range(0, W_tile - patch_size + 1, patch_size):
                # seq : (T, C, pH, pW) → aplatir en (T, C*pH*pW) pour iTransformer
...
# ============================================================
# Principe : au lieu de traiter chaque pas de temps comme un token
# (standard Transformer), iTransformer traite chaque VARIABLE comme
# un token et encode la dimension temporelle comme embedding.
#
# Input  : (B, T, N_var)  → transpose → (B, N_var, T)
# Projection : (B, N_var, T) → (B, N_var, d_model)   via Linear
# Transformer Encoder sur la dimension N_var (variables comme tokens)
# Output : (B, N_var, d_model) → mean-pool sur N_var → (B, d_model)
# Tête classif : (B, d_model) → (B, num_classes * pH * pW)
# ============================================================

class iTransformerEncoderLayer(nn.Module):
    """Couche Transformer inversée : attention sur la dim des variables."""
...
class CDIiTransformerClassifier(nn.Module):
    """
    iTransformer pour la classification du CDI pixel par pixel.

    Entrée  : (B, T, F)  avec F = N_var * pH * pW
              → reshape vers (B, N_var, T*pH*pW) puis projection linéaire

    On considère chaque indicateur (SPI, SPEI, SM, NDVI, LST) comme
    un token (variable-as-token), l'information temporelle + spatiale
    du patch est encodée dans l'embedding de chaque variable.
    """
    def __init__(self, seq_len, input_size, d_model=D_MODEL,
        ...
        # Taille de l'embedding par variable :
        # chaque variable a T pas de temps × pH × pW valeurs spatiales
        embed_per_var = seq_len * patch_size * patch_size  # T * pH * pW

        # Projection d'entrée : embed_per_var → d_model (par variable)
        self.input_proj = nn.Linear(embed_per_var, d_model)

        # Encodeur Transformer inversé (variables comme tokens)
        ...
        # Tête de classification : prédit num_classes * pH * pW par pixel
    ...
    def forward(self, x):
        # x : (B, T, F)  avec F = num_vars * pH * pW
        B, T, F = x.shape
        pH = pW = self.patch_size
        V  = self.num_vars

        # Reshape → (B, V, T * pH * pW) : chaque variable comme token
        # x est de forme (B, T, V * pH * pW)
        x = x.view(B, T, V, pH * pW)           # (B, T, V, pH*pW)
        x = x.permute(0, 2, 1, 3)              # (B, V, T, pH*pW)
        x = x.reshape(B, V, T * pH * pW)       # (B, V, T*pH*pW)

        # Projection linéaire : T*pH*pW → d_model
        x = self.input_proj(x)                  # (B, V, d_model)

        # Encodeur iTransformer : attention sur les variables
        for layer in self.encoder:
            x = layer(x)                        # (B, V, d_model)

        x = self.norm(x)                        # (B, V, d_model)

        # Agrégation sur les variables (mean pooling)
        x = x.mean(dim=1)                       # (B, d_model)
        x = self.dropout(x)

        # Tête classif → (B, num_classes * pH * pW)
        logits = self.classifier(x)             # (B, num_classes * pH * pW)

        # Reshape → (B, num_classes, pH, pW)
...
def process_tile(temporal_data, tile_idx, r0, r1, c0, c1):
    seqs, tgts, dates, origins = prepare_sequences_tile(
    ...
    X_tr_s, X_va_s, X_te_s = normalize_features(X_tr, X_va, X_te)
    del X_tr, X_va, X_te; gc.collect()

    # input_size = C * pH * pW (taille aplatie de la dernière dim)
    input_size = X_tr_s.shape[2]

    tr_loader = DataLoader(CDIDataset(X_tr_s, y_tr), BATCH_SIZE, shuffle=False, pin_memory=True)
    va_loader = DataLoader(CDIDataset(X_va_s, y_va), BATCH_SIZE, shuffle=False, pin_memory=True)
    te_loader = DataLoader(CDIDataset(X_te_s, y_te), BATCH_SIZE, shuffle=False, pin_memory=True)
    del X_tr_s, X_va_s, X_te_s; gc.collect()

    model = CDIiTransformerClassifier(
...
def print_global_summary(results, full_shape, output_dir='.'):
    ...
    print("\n" + "="*70)
    print(f"RESUME GLOBAL (iTransformer) — {len(df)} tuile(s)  |  {total_samples:,} pixels test")
    print("="*70)
    ...
    df.to_csv(os.path.join(output_dir, 'tile_results_itransformer.csv'), index=False)
    print("\nRésultats détaillés → tile_results_itransformer.csv")
...
if __name__ == '__main__':
    OUTPUT_DIR = '/kaggle/working/cdi_rasters_itransformer'
    os.makedirs(OUTPUT_DIR, exist_ok=True)

    print("\n" + "="*70)
    print("🚀 CDI PREDICTION — iTransformer (raster Tunisie complet)")
    print(f"   Nombre de tuiles souhaité : {NUM_TILES}")
    print(f"   Taille des patchs         : {PATCH_SIZE}×{PATCH_SIZE}")
    print(f"   d_model                   : {D_MODEL}")
    print(f"   Têtes d'attention         : {N_HEADS}")
    print(f"   d_ff                      : {D_FF}")
    print("="*70)
    ...
    print("\n🔄 Traitement des tuiles (iTransformer)...\n")
    all_results = []
