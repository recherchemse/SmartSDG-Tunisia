...
# ============================================================
# SÉQUENCES SPATIO-TEMPORELLES POUR UNE TUILE (LSTM)
# Chaque patch est aplati en vecteur : T x (C * pH * pW)
# ============================================================
def prepare_sequences_tile_lstm(temporal_data, r0, r1, c0, c1, patch_size=PATCH_SIZE):
    ...
    for target_idx in range(SEQUENCE_LENGTH, num_months):
        ...
        for pr in range(0, H_tile - patch_size + 1, patch_size):
            for pc in range(0, W_tile - patch_size + 1, patch_size):
                # seq : (T, C, pH, pW) → aplatir en (T, C*pH*pW) pour le LSTM
...
# ============================================================
def normalize_features_lstm(X_train, X_val, X_test):
...
# ============================================================
# MODÈLE LSTM + TÊTE DE CLASSIFICATION PIXEL
# ============================================================
class CDILSTMClassifier(nn.Module):
...
def process_tile(temporal_data, tile_idx, r0, r1, c0, c1):
    seqs, tgts, dates, origins = prepare_sequences_tile_lstm(
    ...
    X_tr_s, X_va_s, X_te_s = normalize_features_lstm(X_tr, X_va, X_te)
    del X_tr, X_va, X_te; gc.collect()

    input_size = X_tr_s.shape[2]  # C * pH * pW

    tr_loader = DataLoader(CDIDatasetLSTM(X_tr_s, y_tr), BATCH_SIZE, shuffle=False, pin_memory=True)
    va_loader = DataLoader(CDIDatasetLSTM(X_va_s, y_va), BATCH_SIZE, shuffle=False, pin_memory=True)
    te_loader = DataLoader(CDIDatasetLSTM(X_te_s, y_te), BATCH_SIZE, shuffle=False, pin_memory=True)
    del X_tr_s, X_va_s, X_te_s; gc.collect()

    model = CDILSTMClassifier(
        input_size   = input_size,
        hidden_size  = HIDDEN_SIZE,
        num_layers   = NUM_LAYERS,
        num_classes  = 6,
...
def print_global_summary(results, full_shape, output_dir='.'):
    ...
    print("\n" + "="*70)
    print(f"RESUME GLOBAL (LSTM) — {len(df)} tuile(s)  |  {total_samples:,} pixels test")
    print("="*70)
    ...
    df.to_csv(os.path.join(output_dir, 'tile_results_lstm.csv'), index=False)
    print("\nRésultats détaillés → tile_results_lstm.csv")
...
if __name__ == '__main__':
    OUTPUT_DIR = '/kaggle/working/cdi_rasters_lstm'
    os.makedirs(OUTPUT_DIR, exist_ok=True)

    print("\n" + "="*70)
    print("🚀 CDI PREDICTION — LSTM (raster Tunisie complet)")
