...
# Hyper-paramètres
SEQUENCE_LENGTH = 3
NUM_CHANNELS    = 64      # Nombre de filtres TCN par couche
NUM_LEVELS      = 4       # Nombre de blocs résiduels TCN
KERNEL_SIZE     = 3       # Taille du noyau de convolution temporelle
DROPOUT         = 0.3
EPOCHS          = 20
BATCH_SIZE      = 32
LEARNING_RATE   = 0.001
...
# ============================================================
# SÉQUENCES SPATIO-TEMPORELLES POUR UNE TUILE (TCN)
# Chaque patch est aplati en vecteur : T x (C * pH * pW)
# ============================================================
def prepare_sequences_tile_tcn(temporal_data, r0, r1, c0, c1, patch_size=PATCH_SIZE):
    ...
    for target_idx in range(SEQUENCE_LENGTH, num_months):
        ...
        for pr in range(0, H_tile - patch_size + 1, patch_size):
            for pc in range(0, W_tile - patch_size + 1, patch_size):
                # seq : (T, C, pH, pW) → aplatir en (T, C*pH*pW) pour le TCN
...
# ============================================================
def normalize_features_tcn(X_train, X_val, X_test):
...
# DATASET
# ============================================================
class CDIDatasetTCN(Dataset):
    def __init__(self, X, y):
        # TCN attend (N, F, T) — on transpose ici
        self.X = torch.FloatTensor(X).permute(0, 2, 1)  # (N, F, T)
...
# ============================================================
# BLOC RÉSIDUEL TCN (Temporal Convolutional Network)
# ============================================================
class TemporalBlock(nn.Module):
    """
    Bloc résiduel TCN avec convolutions dilatées causales.
    Références : Bai et al., "An Empirical Evaluation of Generic
...
class Chomp1d(nn.Module):
    """Supprime les pas futurs introduits par le padding causal."""
...
# ============================================================
# MODÈLE TCN + TÊTE DE CLASSIFICATION PIXEL
# ============================================================
class CDITCNClassifier(nn.Module):
    def __init__(self, input_size, num_channels=NUM_CHANNELS,
                 num_levels=NUM_LEVELS, kernel_size=KERNEL_SIZE,
                 num_classes=6, dropout=DROPOUT, patch_size=PATCH_SIZE):
        """
        Args:
            input_size  : F = C * pH * pW  (dimension des features par pas de temps)
            num_channels: nombre de filtres dans chaque couche TCN
            num_levels  : nombre de blocs résiduels
        ...
        # Construction du réseau TCN empilé avec dilatation exponentielle
        layers = []
        ...
        # Tête de classification : prédit num_classes * pH * pW valeurs
    ...
    def forward(self, x):
        # x : (B, F, T)  — format attendu par Conv1d
        out    = self.tcn(x)          # (B, num_channels, T)
        h_last = out[:, :, -1]        # (B, num_channels)  — dernier pas de temps
...
def process_tile(temporal_data, tile_idx, r0, r1, c0, c1):
    seqs, tgts, dates, origins = prepare_sequences_tile_tcn(
    ...
    X_tr_s, X_va_s, X_te_s = normalize_features_tcn(X_tr, X_va, X_te)
    del X_tr, X_va, X_te; gc.collect()

    input_size = X_tr_s.shape[2]  # C * pH * pW

    tr_loader = DataLoader(CDIDatasetTCN(X_tr_s, y_tr), BATCH_SIZE, shuffle=False, pin_memory=True)
    va_loader = DataLoader(CDIDatasetTCN(X_va_s, y_va), BATCH_SIZE, shuffle=False, pin_memory=True)
    te_loader = DataLoader(CDIDatasetTCN(X_te_s, y_te), BATCH_SIZE, shuffle=False, pin_memory=True)
    del X_tr_s, X_va_s, X_te_s; gc.collect()

    model = CDITCNClassifier(
...
def print_global_summary(results, full_shape, output_dir='.'):
    ...
    print("\n" + "="*70)
    print(f"RESUME GLOBAL (TCN) — {len(df)} tuile(s)  |  {total_samples:,} pixels test")
    print("="*70)
    ...
    df.to_csv(os.path.join(output_dir, 'tile_results_tcn.csv'), index=False)
    print("\nRésultats détaillés → tile_results_tcn.csv")
...
if __name__ == '__main__':
    OUTPUT_DIR = '/kaggle/working/cdi_rasters_tcn'
    os.makedirs(OUTPUT_DIR, exist_ok=True)

    print("\n" + "="*70)
    print("🚀 CDI PREDICTION — TCN (raster Tunisie complet)")
    print(f"   Nombre de tuiles souhaité : {NUM_TILES}")
    print(f"   Taille des patchs         : {PATCH_SIZE}×{PATCH_SIZE}")
    print(f"   Canaux TCN                : {NUM_CHANNELS}")
    print(f"   Niveaux TCN               : {NUM_LEVELS}")
    print(f"   Taille noyau              : {KERNEL_SIZE}")
    print("="*70)
